...
from hermes_mcp_gateway.auth import MCPAccessClaims, verify_mcp_access_token
...
async def filesystem_read(access_token: str, path: str, ctx: Context) -> str:
    ...
    claims = _authorize(access_token, "filesystem")
    mcp_id = claims.require("filesystem")
...
async def database_query(access_token: str, sql: str, ctx: Context) -> str:
    ...
    claims = _authorize(access_token, "database")
    mcp_id = claims.require("database")
    started_at = _timestamp()
...
def _authorize(access_token: str, capability: str) -> MCPAccessClaims:
    claims = verify_mcp_access_token(access_token, SIGNING_KEY)
