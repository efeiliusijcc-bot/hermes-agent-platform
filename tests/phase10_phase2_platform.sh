...
cleanup() {
  api_curl -sS -o /dev/null -X DELETE "$API_URL/api/agents/$AGENT_ID" || true
  api_curl -sS -o /dev/null -X DELETE "$API_URL/api/skills/$SKILL_ID" || true
  api_curl -sS -o /dev/null -X DELETE "$API_URL/api/mcp-servers/$MCP_ID" || true
  rm -rf -- "$TMP_DIR"
}
...
skill=$(api_curl -fsS -X POST "$API_URL/api/skills/upload" -F "file=@$TMP_DIR/skill.zip;type=application/zip")
...
mcp=$(api_curl -fsS -X POST "$API_URL/api/mcp-servers" -H 'Content-Type: application/json' --data "{\"id\":\"$MCP_ID\",\"name\":\"Phase 2 Filesystem MCP\",\"endpoint\":\"http://mcp-gateway:8090/mcp\",\"permission\":\"read_only\",\"config\":{\"kind\":\"filesystem\",\"read_only\":true}}")
...
mcp_updated=$(api_curl -fsS -X PUT "$API_URL/api/mcp-servers/$MCP_ID" -H 'Content-Type: application/json' --data '{"name":"Phase 2 Filesystem MCP Updated","endpoint":"http://mcp-gateway:8090/mcp","permission":"read_only","config":{"kind":"filesystem","read_only":true}}')
...
mcp_test=$(api_curl -fsS -X POST "$API_URL/api/mcp-servers/$MCP_ID/test")
...
api_curl -fsS -X POST "$API_URL/api/agents" -H 'Content-Type: application/json' --data "{\"id\":\"$AGENT_ID\",\"name\":\"Phase 2 Public Agent\",\"role\":\"Structured response Agent\",\"system_prompt\":\"Return only JSON with summary and recommendations fields.\",\"model_config\":{},\"status\":\"active\"}" >/dev/null
schema=$(api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/schema" -H 'Content-Type: application/json' --data '{"input_schema":{"topic":{"type":"string","required":true}},"output_schema":{"summary":"string","recommendations":"array"}}')
...
bad_configuration=$(api_curl -sS -o /dev/null -w '%{http_code}' -X PUT "$API_URL/api/agents/$AGENT_ID/configuration" -H 'Content-Type: application/json' --data "{\"system_prompt\":\"Return only JSON.\",\"model\":\"$MODEL_ID\",\"prompt_template\":\"{{undeclared}}\",\"model_adapter\":\"$MODEL_ADAPTER\",\"model_config\":{}}")
test "$bad_configuration" = "422"
configuration=$(api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/configuration" -H 'Content-Type: application/json' --data "{\"system_prompt\":\"Return only JSON matching the output contract.\",\"model\":\"$MODEL_ID\",\"prompt_template\":\"OUTPUT_JSON_OK topic={{topic}}\",\"model_adapter\":\"$MODEL_ADAPTER\",\"model_config\":{\"temperature\":0}}")
...
mode=$(api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/response-mode" -H 'Content-Type: application/json' --data '{"response_mode":"stream"}')
printf '%s' "$mode" | python3 -c 'import json,sys; assert json.load(sys.stdin)["response_mode"]=="stream"'
api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/skills/$SKILL_ID" >/dev/null
api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/mcp-servers/$MCP_ID" >/dev/null

secret=$(api_curl -fsS -X POST "$API_URL/api/agents/$AGENT_ID/publication/api-key")
api_key=$(printf '%s' "$secret" | python3 -c 'import json,sys; print(json.load(sys.stdin)["api_key"])')
test -n "$api_key"
version=$(api_curl -fsS -X POST "$API_URL/api/agents/$AGENT_ID/versions" -H 'Content-Type: application/json' --data '{"created_by":"offline-validation"}')
...
api_curl -fsS -X PATCH "$API_URL/api/agents/$AGENT_ID/versions/$VERSION_ID/status" -H 'Content-Type: application/json' --data '{"status":"testing"}' >/dev/null
api_curl -fsS -X PATCH "$API_URL/api/agents/$AGENT_ID/versions/$VERSION_ID/status" -H 'Content-Type: application/json' --data '{"status":"release_candidate"}' >/dev/null
api_curl -fsS -X POST "$API_URL/api/agents/$AGENT_ID/publish" -H 'Content-Type: application/json' --data "{\"version\":\"$VERSION_ID\"}" >/dev/null
...
locked_configuration=$(api_curl -sS -o /dev/null -w '%{http_code}' -X PUT "$API_URL/api/agents/$AGENT_ID/configuration" -H 'Content-Type: application/json' --data "{\"system_prompt\":\"Published versions are immutable.\",\"model\":\"$MODEL_ID\",\"prompt_template\":\"{{topic}}\",\"model_adapter\":\"$MODEL_ADAPTER\",\"model_config\":{}}")
...
disabled=$(api_curl -fsS -X PUT "$API_URL/api/agents/$AGENT_ID/publication" -H 'Content-Type: application/json' --data '{"status":"disabled"}')
